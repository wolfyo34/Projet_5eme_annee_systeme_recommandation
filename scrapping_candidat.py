...
username.send_keys("rtjrtjrtjktrk@gmail.com")
time.sleep(0.5)
   ...
   if check_exists_by_xpath("/html/body/div[7]/div[3]/div/div/div/div/div[3]/div/div/main/div/div/div[6]/div/section/div[2]/button") ==True:
       ...
       while i <len(description):
            ...
            if "Endorsed" in description[i]:
                del description[i]
            i+=1
       C = {'name': [name],'job_title': [job_title],'competence': [description]}
       donnees = DataFrame(C, columns= ['name', 'job_title', 'competence'])
       export_csv = donnees.to_csv ("candidat.csv", mode='a',index = None, header=False, encoding='utf8', sep=';')
